...
<div><center><h3>Next Generation Learning Platform Installation</h3></center></div><br>
<div id="container">
	<div id="steps">
	<ul class="steps">
<?php 
...
?>
		...
		<div id="footer" style="height: 18px">
			<div style="float: left">&copy; <a href="http://hardmagic.com" target=blank>HardMagic</a> <?php echo date('Y'); ?> </div> <div style="float: right"><div class="logo_fave"></div> Pedadida Setup Wizard</div>
		</div>
	</div>
</div>
...
<?php
		...
		/**
		 *  Find out What type of Server is being used.
		 */
		function AddDatabaseTypeDropdown($elementName, $selectedValue="mysqli")
		{
			$strblock = '<select name="'.$elementName.'">
<option value="pgsql">Postgre SQL Server (PGSQL)</option>
<option value="mysqli">MySQLi Server (MYSQLI)</option>
<option value="mssql">Microsoft MSSQL Server (MSSQL)</option>
<option value="sqlsrv">Microsoft SQL Server (SQLSRV)</option>
<option value="oci">Oracle Call Interface (OCI)</option>
</select>';
	}
