...
ini_set('max_execution_time', 330);
    ...
    *  ================================================================= */   
	...
	// Used for Password       
    require_once('assets'.DIRECTORY_SEPARATOR.'class-phpass.php'); 
    ...
    else
    {
        ...
        {   			
			...
			if($steps[STEP_IOFILES]['enabled'])
			{
				...
				$file = $steps[STEP_WRITECONFIG]['maskname'];
			...
			*/
			...
			if($steps[STEP_DBPREFIX]['enabled'] && ($validPrefix == false || $step == STEP_DBPREFIX))
			{
				...
				// if prefix is valid - otherwise the user is seeing this for
				...
				$page->ShowPage(STEP_DBPREFIX);                     
					...
					/* ===================================================[ PEDADIDA EXTRA SETTINGS SELECTION ]=================================================== */
			...
			*/
			...
			$page->Label('Use SSL for Library?');
			$page->FormInput($keywords['special']['library_secure'], 'library_secure');
				
			$page->Label('Bin Path');
			$page->FormInput($keywords['special']['bin_path'], 'bin_path');
			
			$page->Label('Imagemagick Path:');
				$page->FormInput($keywords['special']['imagemagick_path'], 'imagemagick_path');
			
			$page->Label('FFMPEG Path:');
				$page->FormInput($keywords['special']['ffmpeg_path'], 'ffmpeg_path');
			
			$page->Label('Exiftool Path:');
			$page->FormInput($keywords['special']['exiftool_path'], 'exiftool_path');
			
			$page->Label('Antiword Path:');
			$page->FormInput($keywords['special']['antiword_path'], 'antiword_path');
			
			$page->Label('Ghostscript Path:');
			$page->FormInput($keywords['special']['ghostscript_path'], 'ghostscript_path');
			
			$page->Label('PHP Path:');
			$page->FormInput($keywords['special']['php_path'], 'php_path');
			
			$page->Label('Library Name:');
			$page->FormInput($keywords['special']['library_name'], 'library_name');
			
			$page->Label('Notification Email Address:');
				$page->FormInput($keywords['special']['email_notify'], 'email_notify');
				
			$page->Label('Classroom Data Path (folder where Classroom Data can be stored.  Should be private and inaccessible from Public):');
				$page->FormInput($keywords['special']['classroom_data'], 'classroom_data');
				
				$page->Label('Classroom Name:');
				$page->FormInput($keywords['special']['class_name'], 'class_name');
				
				$page->Label('Lab Name:');
				$page->FormInput($keywords['special']['lab_name'], 'lab_name');
				
				$page->Label('Lab File Storage Path:');
				$page->FormInput($keywords['special']['lab_storage'], 'lab_storage');
			
		
				$prev = GetPrevStep(STEP_CONFIGEXTRAS);
				if($prev) $page->FormButton('Back', array('step'=>$prev));   
                   
                $page->FormSubmit('Next');
                $page->FormClose();
				$page->ShowPage(STEP_CONFIGEXTRAS);
			}	
			
			
							/* ===================================================[ Hidden Keys SELECTION ]=================================================== */
			...
			*/
			...
			if($steps[STEP_HIDDENKEYS]['enabled'] && $step == STEP_HIDDENKEYS)
			{
				$page->MainTitle($steps[STEP_HIDDENKEYS]['title'], 'hiddenkeys');

				$page->Paragraph('For Security, some parts of the site use complex Special Keys.  You may use whatever you like, though we suggest getting these keys from this site: <a href="https://api.wordpress.org/secret-key/1.1/salt/." target="_blank">Wordpress Salts</a>  Do <b>nOT</b> use the keys already provided - they are examples only.');
				
				
				$page->FormStart(array('step'=>GetNextStep(STEP_HIDDENKEYS)));
				
				$page->Label('Special Key 1:');
				$page->FormInput($keywords['special']['key1'], 'key1');
				
				$page->Label('Special Key 2:');
				$page->FormInput($keywords['special']['key2'], 'key2');
				
				$page->Label('Special Key 3:');
				$page->FormInput($keywords['special']['key3'], 'key3');
				
				$page->Label('Special Key 4:');
				$page->FormInput($keywords['special']['key4'], 'key4');
				
				$page->Label('Special Key 5:');
				$page->FormInput($keywords['special']['key5'], 'key5');
				
				$page->Label('Special Key 6:');
				$page->FormInput($keywords['special']['key6'], 'key6');
				
				$page->Label('Special Key 7:');
				$page->FormInput($keywords['special']['key7'], 'key7');
				
				$page->Label('Special Key 8:');
				$page->FormInput($keywords['special']['key8'], 'key8');
		
				$prev = GetPrevStep(STEP_HIDDENKEYS);
				if($prev) $page->FormButton('Back', array('step'=>$prev));   
                   
                $page->FormSubmit('Next');
                $page->FormClose();
				$page->ShowPage(STEP_HIDDENKEYS);
			}	
			...
			if($steps[STEP_ROOTUSER]['enabled'] && (!AdminAccountSuccess() || $step == STEP_ROOTUSER))
			{
				...
				// Simplify the code a little bit with smaller variable
				$data = $keywords['admin'];
					...
					&& $_REQUEST['create'] == 'administrator')
				{
					...
					/**************************************************************************************
					*                                                                                     *
					*             -RULES- OF CREATING ADMINISTRATOR ACCOUNTS         *
					*                                                                                     *
					**************************************************************************************/
			...
			if($steps[STEP_RUNSQL]['enabled'] && (count($successInstall) == 0 || $step == STEP_RUNSQL))
			{
				...
				*  step STEP_DBPREFIX is previous step from STEP_RUNSQL!  */					
				...
				if(count($tableList) > 0) 
				{
					...
					{
						$page->Paragraph('You should see wp_users above as the only table so far.  Our user table is synchronized with the WordPress User Database with a few extra columns.');
						$page->Paragraph('Pedadida is 100% compatible with Wordpress, even though we modify their tables.');
					}
				}
				else
				{
